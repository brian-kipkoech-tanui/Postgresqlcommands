...
-- Group by region
GROUP BY s15.region
-- Include only those regions whose countries are below 100 on avg
HAVING(ROUND(AVG(s15.happiness_rank),2) >= 100 AND
	   ROUND(AVG(s16.happiness_rank),2) >= 100 AND
	   ROUND(AVG(s17.happiness_rank),2) >= 100 AND
	   ROUND(AVG(s18.overall_rank),2) >= 100   AND
	   ROUND(AVG(s19.overall_rank),2) >= 100)
